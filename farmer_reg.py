...
cur = conn.cursor()
q1="""select max(id) from farmer_reg_details"""
cur.execute(q1)
r=cur.fetchone()
if r[0]==None:
    n=0
...
margin: 0;
padding: 0;
background-image:url("images/ac6.jfif");
...
<script>
 function validate()
 {
  ...
  e=document.getElementById("qual");
 ...
 <div class="container-fluid" class="jumbotron text-center">
		<div class="row well">
			  ...
			  <div class="col-sm-7"><h1 width="100%"align="center">AGRI HELPLINE</h1></div>
...
print("""<tr><td>ID:</td><td><input type="text" name="farmerid" readonly value='%s'></td></tr>"""%(farmerid))
...
if psub!=None:
        ...
        if pphto.filename:
                ...
                q1="""INSERT INTO farmer_reg_details(farmer_id,name,dob,gender,qualification,address,district,state,email,contactno,createpassword,confirmpassword,photoupload) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" %(pfarid,pname,pdob,pgen,pqual,padd,pdist,pstate,pemail,pcontact,pcreate,pconfirm,fn)
