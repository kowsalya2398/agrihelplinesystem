...
f=cgi.FieldStorage()
pid=f.getvalue('id')
q1="""select * from student_reg_details where id=%s""" %(pid)
cur.execute(q1)
r=cur.fetchone()


if r!=None:
    q2= """delete from student_reg_details where id=%s""" %(pid)
    cur.execute(q2)
    conn.commit()
    print("""<script>alert("data deleted sucessfully");
    location.href="adm_student_delete.py?id=%s"
    </script>""" %(pid))
...
q3="""select * from student_reg_details"""
try:
    cur.execute(q3)
    res=cur.fetchall()
except:
